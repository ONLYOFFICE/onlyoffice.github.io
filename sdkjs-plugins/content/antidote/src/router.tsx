...
function parseUrl(url: string): Pick<AppLocation, 'path' | 'query'> {
...
export function LocationProvider({ children }: { children: ComponentChildren }) {
  ...
  const value = useMemo<AppLocation>(() => ({ ...parseUrl(url), route: setUrl }), [url]);
  return <LocationContext.Provider value={value}>{children}</LocationContext.Provider>;
}

export function useLocation(): AppLocation {
  const location = useContext(LocationContext);
  if (!location) throw new Error('useLocation must be used within a LocationProvider');
  return location;
}
