...
type SlideMethodArgs = {
   ...
   * @since 9.5.0
   */
  AnnotateParagraph: [data: { type?: string; name?: string; paragraphId?: string; recalcId?: string; ranges?: object[] }];
  /**
   ...
   * @since 9.4.0
   */
  ApplyTheme: [themeIndex: number | string];
  /**
   ...
   *
   * @returns Array of theme objects with "index" and "name" properties.
   * @since 9.4.0
   */
  GetEditorThemes: [];
  /**
   ...
   *
   * @param slideNumber - The slide index.
   * @since 9.0.0
   */
   ...
   *
   * @param nSlideIndex - The slide index.
   * @since 8.0.0
   */
   ...
   *
   * @param url - The URL of the source presentation (pptx).
   * @param options - Insertion options.
   * @param token - JWT signed with the Document Server inbox secret, authorizing the URL download. Required in
   *   deployments with browser-side JWT enabled.
   * @returns The number of slides that were inserted. Returned asynchronously.
   * @since 9.5.0
   */
  InsertPresentationFromUrl: [url: string, options?: { index?: number; slideStart?: number; slideEnd?: number; keepSourceFormatting?: boolean }, token?: string];
  /**
   ...
   * @since 9.5.0
   */
  RemoveAnnotationRange: [annotation: { all?: boolean }];
  /**
   ...
   *
   * @param internalId - The OLE object identifier which is used to work with OLE object added to the presentation.
   * @since 9.1.0
   */
   ...
   * @since 9.5.0
   */
  SelectAnnotationRange: [annotation: object];
  /**
...
type SlideMethodReturnMap = {
  ...
  GetVersion: string;
  GoToNextSlideInSlideShow: any;
  GoToPreviousSlideInSlideShow: any;
  GoToSlide: any;
  GoToSlideInSlideShow: any;
  InputText: any;
  InsertPresentationFromUrl: number;
...
export type { SlideMethodArgs, SlideMethodName, SlideMethodReturn };
